...
export default {
  ...
  created() {
    this.$eventBus.$on('calculationDone', () => {
      console.log("⚛ ~ calculationDonedata");
    })
  },
  mounted () {
    return
    ...
    // for (let j = 1; j < 11; j++) {
    //   $('table tr#tab-' + i).append("<td>" + j + "</td>")
    // }
    // for (let i = 1; i < 11; i++) {
    //   $('table').append("<tr id='tab-" + i + "'></tr>");
    //   $('table tr#tab-' + i).append("<td>" + i + "</td>")
    //   for (let j = 1; j < 11; j++) {
    //     $('table tr#tab-' + i).append("<td class='td-" + j + "'>0</td>")
    //   }
    // }

    const myTable = document.querySelector('table')
    myTable.insertAdjacentHTML("beforeend", "<tr id='tab-" + i + "'></tr>");
